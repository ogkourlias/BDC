...
def read(spark):
    """Run function"""
    df = spark.read.csv("/data/datasets/EBI/interpro/refseq_scan/bacteria.nonredundant_protein.1.protein.faa.tsv", sep=r'\t', header=False)
    return df
...
def get_top_10_percentage(df):
    ...
    subset = with_span.sort("span", ascending=False).select(col("_c11"), col("span"),
                                                            col("_c12")).where(col("span") >= 0.9)
    return subset
