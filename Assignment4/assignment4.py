...
def parse_arguments():
    ...
    argparser = ap.ArgumentParser(
        description="Calculate PHRED scores over the network."
    )
    ...
    server_args.add_argument(
        ...
        dest="csvfile",
        type=Path,
        help="CSV file to store the output. Defaults to STDOUT.",
    )
    server_args.add_argument(
        "fastq_files", type=Path, nargs="*", help="At least one FastQ file to process."
    )
    return argparser.parse_args()


class PhredScoreCalculator:
    """Phred score calculator class."""
    ...
    def divide_work(self, score_lines, chunk_size):
        ...
        return [
            score_lines[i : i + chunk_size]
            for i in range(0, len(score_lines), chunk_size)
        ]
    ...
    def combine_results(self, results):
        ...
        scores_arrays = [
            np.pad(result[0], (0, max_len - len(result[0])), "constant")
            for result in results
        ]
        counts_arrays = [
            np.pad(result[1], (0, max_len - len(result[1])), "constant")
            for result in results
        ]
